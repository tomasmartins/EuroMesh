...
typedef struct {
    ...
    uint8_t bandwidth_bits;    /* 4-bit field: maps to BW[3:0] in RegModemConfig1 */
    uint8_t spreading_factor;  /* 4-bit field: SF6–SF12 */
    uint8_t coding_rate_bits;  /* 3-bit field: CR4/5=1 … CR4/8=4 */
    int8_t tx_power_dbm;
    bool implicit_header_mode;
    bool use_pa_boost;         /* true = PA_BOOST pin (up to 17 dBm); false = RFO pin (up to 14 dBm) */
