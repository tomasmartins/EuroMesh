...
{
    return beacon_start_ms + TDMA_BEACON_SLOT_MS;
}

uint32_t tdma_reg_window_end_ms(uint32_t beacon_start_ms)
{
    return beacon_start_ms + TDMA_BEACON_SLOT_MS + TDMA_REG_SLOT_MS;
}

bool tdma_is_in_reg_window(uint32_t beacon_start_ms, uint32_t now_ms)
{
    uint32_t start = tdma_reg_window_start_ms(beacon_start_ms);
